...
const ToxicoSet = (props) => {
    const { getDataset, getHeader, dataset } = useSingleDataset(dataTypes.toxicogenomics, `${props.params.id1}/${props.params.id2}`);
        ...
        <div className='pageContent'>
            {
                ...
                <React.Fragment>
                    { getHeader() }
                    <GeneralInfoAccordion datasetType='ToxicoSet' data={dataset.data.generalInfo}/>
                    <TabContainer>
            ...
            { !dataset.data && <h3>ToxicoSet with the specified DOI could not be found</h3>}
        </div>
    );
}
